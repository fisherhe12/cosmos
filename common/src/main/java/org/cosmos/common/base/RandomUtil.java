 ...
 * RandomUtil，但默认使用性能较优的ThreadLocalRandom，并可配置其他的Random)
 * <p>
 * 3. 随机字符串
 */
public class RandomUtil {
    ...
    private static void nonNegative(Number x) {
        if (!(x.doubleValue()>= 0)) { // not x < 0, to work with NaN.
            throw new IllegalArgumentException("min" + " (" + x + ") must be >= 0");
        }
    }
